...
description: "The methodology behind Advizr's 5x ROI guarantee: baseline capture before any build, measured time savings after, and a monthly report you can audit."
---
...
A guarantee is only as good as its measurement. This page documents exactly how we calculate the return behind our [5x ROI guarantee](/docs/services/guarantees), so you can audit every number we report.
...
Every system we deploy logs its own activity (runs, items processed, outcomes). Monthly, your FDE reports value delivered against the baseline: hours displaced, what those hours are worth at your own loaded cost, and the running ROI multiple. The numbers come from system logs, not estimates — you can see the run history yourself in [your dashboard](/docs/platform/features/runs-and-workflows).
...
If the measured multiple is below 5x at day 30 after deployment, the guarantee triggers: we keep working at no charge until it isn't. Individual results depend on your workflows, volumes, and labour costs.

## Estimate yours

The [ROI worksheet](/docs/resources/templates/roi-calculator) walks the same arithmetic in hours before an engagement exists, so you can decide whether this is worth a conversation at all.
